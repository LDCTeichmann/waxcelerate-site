 ...
 * ein Wachs-Punkt auf derselben Euro-Achse, dazwischen die Differenz als Pfeil
 * von Oel nach Wachs. Zeigt in einem Blick, WOHER die Ersparnis kommt — und
 * dass Wachs beim Schmierstoff selbst teurer ist (Ocker, Pfeil nach rechts).
 * Unter der Haarlinie die Summe: dieselbe Zahl wie die Antwort der Karte.
 */
export function CostDumbbell({ rows, oilLabel, waxLabel, perYear, eur, net }: {
  rows: CostRow[]; oilLabel: string; waxLabel: string; perYear: string;
  eur: (n: number) => string;
  /** Wachs minus Oel im Jahr (negativ = Ersparnis), aus der Rechnung durchgereicht. */
  net?: number;
}) {
  const { t } = useLanguage();
  const sh = t.tools.shared;
  ...
  const id = useSvgId();
  const shown = useRevealOnce(ref);
  const reveal = useTween(shown ? 1 : 0, 900);
  const vals = useTweenList(rows.flatMap(r => [r.oil, r.wax]));
  const ROW = 34, TOP = 24, LABEL_W = 84, DELTA_W = 48, PAD = 4;
  const sumY = TOP + rows.length * ROW + 4;
  const H = sumY + 22;
  const x0 = LABEL_W, x1 = W - DELTA_W - PAD;
  const max = Math.max(1, ...vals) * 1.04;
  const x = (v: number) => x0 + (v / max) * (x1 - x0);
  const total = net ?? rows.reduce((s, r) => s + r.wax - r.oil, 0);
  const signed = (d: number) => (Math.round(d) === 0 ? '±0' : `${d < 0 ? '−' : '+'}${eur(Math.abs(d))}`);
    ...
    <div ref={ref} className="w-full">
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} role="img" className="block"
        ...
        <g style={FONT}>
          ...
          <circle cx={x0 + 62} cy={9} r={5} fill="var(--brand)" />
          <text x={x0 + 71} y={13} fill="var(--txm)">{waxLabel}</text>
          {W >= 300 && <text x={W - PAD} y={13} textAnchor="end" fill="var(--txff)">{perYear}</text>}
        </g>

        {rows.map((r, i) => {
          const cy = TOP + i * ROW + ROW / 2;
          const oil = vals[2 * i] ?? r.oil, wax = vals[2 * i + 1] ?? r.wax;
          // Zeile fuer Zeile versetzt: der Wachs-Punkt waechst aus dem Oel-Punkt.
          const k = Math.min(1, Math.max(0, (reveal - i * 0.12) / 0.76));
          const xo = x(oil), xw = x(oil + (wax - oil) * k);
          const saves = r.wax <= r.oil;
          const tint = saves ? 'var(--brand)' : 'var(--tool-warn)';
            ...
            <g key={r.label} className={id('row')}>
              ...
              <defs>
                <linearGradient id={id(`c${i}`)} gradientUnits="userSpaceOnUse" x1={xo} x2={xw} y1={cy} y2={cy}>
                  ...
                  <stop offset="1" style={{ stopColor: tint }} />
                </linearGradient>
              </defs>
              <line x1={x0} x2={x1} y1={cy} y2={cy} stroke="var(--bd2)" strokeWidth={1} />
              <text x={0} y={cy + 4} style={FONT} fill="var(--tx2)">{r.label}</text>
              {Math.abs(xw - xo) > 1 && (
                <line x1={xo} x2={xw} y1={cy} y2={cy} stroke={url(id, `c${i}`)} strokeWidth={3} strokeLinecap="round" />
              )}
              {Math.abs(xw - xo) > 26 && (
                <path d={`M${xw - dir * 9},${cy} l${-dir * 5},-3.2 v6.4 Z`} fill={tint} />
              )}
              <circle cx={xo} cy={cy} r={5} fill="var(--sf)" stroke={OIL} strokeWidth={2} />
              <circle cx={xw} cy={cy} r={6} fill="var(--brand)" stroke="var(--sf)" strokeWidth={2} />
              {/* Werte an den Punkten: Oel oberhalb, Wachs unterhalb — kollidieren
                  so auch nicht, wenn beide Punkte nah beieinander liegen. */}
              <text x={xo} y={cy - 9} textAnchor="middle" style={{ ...FONT, ...HALO }} fill="var(--txf)">{eur(r.oil)}</text>
              <text x={x(wax)} y={cy + 18} textAnchor="middle" style={{ ...FONT, ...HALO, opacity: k }} fill="var(--tx2)">{eur(r.wax)}</text>
              <text x={W - PAD} y={cy + 4} textAnchor="end" style={FONT} fontWeight={600}
                fill={saves ? 'var(--tx1)' : 'var(--tool-warn)'}>
                {signed(d)}
              </text>
            </g>
          );
        ...
        <line x1={0} x2={W - PAD} y1={sumY} y2={sumY} stroke="var(--bd2)" strokeWidth={1} />
        <text x={0} y={sumY + 17} style={FONT} fontWeight={600} fill="var(--tx2)">
          {total <= 0 ? sh.sketchSaved : sh.sketchExtra}
        </text>
        <text x={W - PAD} y={sumY + 17} textAnchor="end" style={{ ...FONT, fontSize: 13 }} fontWeight={700}
          fill={total <= 0 ? 'var(--tx1)' : 'var(--tool-warn)'}>
          {signed(total)}
        </text>
      </svg>
    </div>

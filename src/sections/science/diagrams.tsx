  ...
  return (
    <Fig vb="0 0 240 120" label="lamellar packing"
      ...
      {[28, 37, 46, 55, 64, 73, 82].map(y => (
        <line key={`d${y}`} x1={26} y1={y} x2={102} y2={y} stroke="var(--accent)" strokeWidth={2.4} strokeLinecap="round" />
      ))}
      ...
      <Lbl x={64} y={104} fill="var(--accent)">{de ? 'Dicht' : 'Dense'}</Lbl>
...
function DropLift({ de }: { de: boolean }) {
  return (
    <Fig vb="0 0 240 120" label="drop point lift"
      ...
      <rect x={138} y={44} width={68} height={12} rx={6} fill="var(--accent)" />
      <Num x={172} y={34} fill="var(--accent)">72–78 °C</Num>
      <line x1={106} y1={78} x2={140} y2={52} stroke="var(--accent)" strokeWidth={1.6} markerEnd="url(#dl-arrow)" />
    </Fig>
  );
}
...
function ColdFlex({ de }: { de: boolean }) {
  return (
    <Fig vb="0 0 240 120" label="cold flexibility"
      ...
      <path d="M28 80 Q64 40 100 80" stroke="var(--accent)" strokeWidth={2.6} fill="none" strokeLinecap="round" />
      <Lbl x={64} y={102} fill="var(--accent)">{de ? 'Elastisch' : 'Elastic'}</Lbl>
...
function Shear({ de }: { de: boolean }) {
  ...
  const Row = ({ y, soft, r, dx = 0 }: { y: number; soft?: boolean; r: number; dx?: number }) =>
    <>{sx.map((x, i) => (
      <circle key={i} cx={x + dx} cy={y} r={r}
        fill={soft ? 'rgba(var(--accent-rgb),0.55)' : 'var(--accent)'}
  ...
  return (
    <Fig vb="0 0 240 150" label="MoS2 layer shear"
      ...
      <line x1={150} y1={12} x2={196} y2={12} stroke="var(--accent)" strokeWidth={1.6} markerEnd="url(#sh-arrow)" />
      {/* van-der-Waals gap */}
      ...
      <line x1={90} y1={138} x2={44} y2={138} stroke="var(--accent)" strokeWidth={1.6} markerEnd="url(#sh-arrow)" />
    </Fig>
  );
}
...
function Density({ de }: { de: boolean }) {
  return (
    <Fig vb="0 0 240 120" label="density ratio"
      ...
      <text x={120} y={50} textAnchor="middle" fontSize={13} fontWeight={600} fill="var(--accent)">5,6×</text>
      <circle cx={186} cy={58} r={36} fill="rgba(var(--accent-rgb),0.10)" stroke="var(--accent)" strokeWidth={2} />
      <Num x={186} y={62} fontSize={13} fill="var(--accent)">5,06</Num>
...
function Radical({ de }: { de: boolean }) {
  return (
    <Fig vb="0 0 240 110" label="radical chain break"
      ...
      <path d="M86 70 Q120 26 154 70" stroke="var(--accent)" strokeWidth={2.6} fill="none" strokeLinecap="round" />
      <path d="M120 40 L120 60" stroke="var(--accent)" strokeWidth={1.6} />
      ...
      <line x1={82} y1={50} x2={92} y2={66} stroke="var(--accent)" strokeWidth={1.6} />
      <text className="num-data" fontSize={10} fill="var(--accent)" x={170} y={50}>MoS₂ ✓</text>
...
function Ptfe({ de }: { de: boolean }) {
  return (
    <Fig vb="0 0 240 120" label="PTFE glide film"
      ...
      <line x1={150} y1={60} x2={200} y2={60} stroke="var(--accent)" strokeWidth={1.6} markerEnd="url(#ptfe-arrow)" />
      <Lbl x={120} y={108} fill="var(--accent)">{de ? 'glatt, nichts haftet' : 'slick, nothing sticks'}</Lbl>
    </Fig>
  );
}
...
function Stearin({ de }: { de: boolean }) {
  return (
    <Fig vb="0 0 240 120" label="adhesion promoter"
      ...
      {[70, 120, 170].map(x => (
        <g key={x}>
          <path d={`M${x} 86 L${x - 6} 70 L${x + 6} 54 L${x - 6} 38`} stroke="var(--accent)" strokeWidth={1.6} fill="none" strokeLinecap="round" strokeLinejoin="round" />
          <circle cx={x} cy={88} r={4} fill="var(--accent)" />
        </g>
      ))}

  ...
  return (
    <Fig vb="0 0 240 120" label="lamellar packing"
      ...
      {[28, 37, 46, 55, 64, 73, 82].map(y => (
        <line key={`d${y}`} x1={26} y1={y} x2={102} y2={y} stroke="var(--accent-soft)" strokeWidth="var(--dw-bold)" strokeLinecap="round" />
      ))}
      <line x1={120} y1={24} x2={120} y2={86} stroke="var(--bd)" strokeWidth="var(--dw-hair)" />
      {[28, 46, 64, 82].map(y => (
        <line key={`g${y}`} x1={138} y1={y} x2={214} y2={y} stroke="var(--tx2)" strokeWidth="var(--dw-bold)" opacity={0.5} strokeLinecap="round" />
      ))}
...
function DropLift({ de }: { de: boolean }) {
  return (
    <Fig vb="0 0 240 120" label="drop point lift"
      ...
      <line x1={24} y1={98} x2={216} y2={98} stroke="var(--bd)" strokeWidth="var(--dw-hair)" />
      ...
      <line x1={106} y1={78} x2={140} y2={52} stroke="var(--accent-soft)" strokeWidth="var(--dw-line)" markerEnd="url(#dl-arrow)" />
    </Fig>
  );
}
...
function ColdFlex({ de }: { de: boolean }) {
  return (
    <Fig vb="0 0 240 120" label="cold flexibility"
      ...
      <path d="M182 54 Q194 48 212 80" stroke="var(--tx2)" strokeWidth="var(--dw-bold)" fill="none" strokeLinecap="round" opacity={0.5} />
      <line x1={179} y1={44} x2={179} y2={64} stroke="var(--txf)" strokeWidth="var(--dw-hair)" />
...
function Shear({ de }: { de: boolean }) {
  return (
    <Fig vb="0 0 240 150" label="MoS2 layer shear"
      ...
      <line x1={150} y1={12} x2={196} y2={12} stroke="var(--accent-soft)" strokeWidth="var(--dw-line)" markerEnd="url(#sh-arrow)" />
      {/* van-der-Waals gap */}
      <line x1={14} y1={75} x2={226} y2={75} stroke="rgba(var(--accent-rgb),0.30)" strokeWidth="var(--dw-hair)" strokeDasharray="6 5" />
      {/* bottom sandwich, shifted left */}
      <ShearRow y={94} soft r={4.5} dx={-10} />
      <ShearRow y={110} r={6.5} dx={-10} />
      <ShearRow y={126} soft r={4.5} dx={-10} />
      <line x1={90} y1={138} x2={44} y2={138} stroke="var(--accent-soft)" strokeWidth="var(--dw-line)" markerEnd="url(#sh-arrow)" />
    </Fig>
  );
}
...
function Ptfe({ de }: { de: boolean }) {
  return (
    <Fig vb="0 0 240 120" label="PTFE glide film"
      ...
      <line x1={24} y1={88} x2={216} y2={88} stroke="var(--accent-soft)" strokeWidth="var(--dw-bold)" strokeLinecap="round" />
      {[56, 98, 140].map((x, i) => (
        <circle key={x} cx={x} cy={62 - i * 2} r={7 - i} fill="none" stroke="var(--tx2)" strokeWidth="var(--dw-line)" opacity={0.6} />
      ))}
      <line x1={150} y1={60} x2={200} y2={60} stroke="var(--accent-soft)" strokeWidth="var(--dw-line)" markerEnd="url(#ptfe-arrow)" />
...
function Stearin({ de }: { de: boolean }) {
  return (
    <Fig vb="0 0 240 120" label="adhesion promoter"
      ...
      {[70, 120, 170].map(x => (
        <g key={x}>
          <path d={`M${x} 86 L${x - 6} 70 L${x + 6} 54 L${x - 6} 38`} stroke="var(--accent-soft)" strokeWidth="var(--dw-line)" fill="none" strokeLinecap="round" strokeLinejoin="round" />

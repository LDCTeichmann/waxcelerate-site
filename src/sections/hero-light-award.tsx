...
export function HeroAward() {
  ...
  const scanRef     = useRef<HTMLDivElement>(null);   // one-time "measurement scan" over the specimen
  const underlineRef = useRef<HTMLSpanElement>(null); // kinetic accent underline under the headline
  ...
  useEffect(() => {
    ...
    // Mess-Scan: ein einmaliger Akzent-Strich fährt über das „Specimen" — die
    // Präzisions-/Lupen-Geste als kinetischer Signature-Moment.
    if (scanRef.current) {
      tl.set(scanRef.current, { opacity: 0 }, 0.95);
      tl.to(scanRef.current, { opacity: 0.85, duration: 0.22, ease: 'power1.out' }, 0.95);
      tl.fromTo(scanRef.current, { top: '-3%' }, { top: '103%', duration: 1.15, ease: 'sine.inOut' }, 0.95);
      tl.to(scanRef.current, { opacity: 0, duration: 0.3, ease: 'power1.in' }, 1.9);
    }
    ...
    // Kinetischer Akzent-Unterstrich „zeichnet sich" unter die Headline.
    if (underlineRef.current) {
      tl.fromTo(underlineRef.current, { scaleX: 0 }, { scaleX: 1, duration: 0.7, ease: 'power3.inOut' }, 1.35);
    }
  ...
  return (
    <section id="home" ref={rootRef} className="hero-editorial relative" style={{ background: 'var(--pg)' }}>
        ...
        >
          ...
          {/* Examination light-pool — definiertes „Untersuchungslicht" hinter dem
              Specimen (Tiefe + Material, award pass), Desktop only */}
          <div
            aria-hidden
            ...
            style={{
              ...
              background: 'radial-gradient(circle at 50% 46%, rgba(240,246,255,0.30) 0%, rgba(206,224,255,0.12) 40%, rgba(46,120,200,0.05) 62%, transparent 76%)',
              filter: 'blur(36px)',
            }}
          />
          ...
          <div
            ...
            className="absolute z-[5] pointer-events-none will-change-transform
                       left-1/2 -translate-x-1/2 top-[205px] -translate-y-1/2
                       w-[clamp(150px,42vw,210px)]
                       lg:left-[53%] lg:top-[43%] lg:translate-y-[-50%] lg:w-[clamp(250px,27vw,410px)]"
          >
            <div ref={cubeAnimRef} className="relative will-change-transform"
              ...
              />
              {/* Floor-Reflexion — gespiegelt + weich nach unten ausgeblendet (Material/Tiefe) */}
              <div
                aria-hidden
                className="absolute left-0 right-0 top-full overflow-hidden pointer-events-none"
                style={{
                  height: '58%', opacity: 0.16, filter: 'blur(2px)',
              ...
              >
                <img src="/images/hero/wax-cutout.webp" alt="" className="block w-full h-auto"
                  style={{ transform: 'scaleY(-1)' }} />
              </div>
            </div>

            {/* Mess-Scan-Strich — einmaliger Sweep über das Specimen (Desktop) */}
            <div
              ref={scanRef}
              aria-hidden
              className="absolute left-[-7%] right-[-7%] pointer-events-none hidden lg:block"
              style={{
                top: '-3%', height: '2px', opacity: 0,
                background: 'linear-gradient(90deg, transparent, rgba(var(--brand-blue-rgb),0.95) 50%, transparent)',
                boxShadow: '0 0 14px rgba(var(--brand-blue-rgb),0.6)',
              }}
            />
          </div>
          ...
          <button
            ...
            style={{ left: '53%', top: '43%', width: 'clamp(250px,27vw,410px)', height: 'clamp(250px,27vw,410px)',
                     transform: 'translate(-50%,-50%)', cursor: lensOn && lensActive ? 'none' : 'pointer' }}
          />

          {/* Specimen-Callouts — Leader-Linien rechts ins dunkle Ketten-Feld, helle
              Mono-Labels (Lab-Signatur; kollidiert nicht mit der Typo links, xl) */}
          {[
            { top: '33%', label: 'MoS₂ · Festschmierstoff' },
            { top: '60%', label: de ? '58–60 °C · Tropfpunkt' : '58–60 °C · drop point' },
          ].map((c) => (
            <div key={c.label} data-hero aria-hidden
                 className="hidden xl:block absolute z-[6] pointer-events-none"
                 style={{ left: '68%', right: 0, top: c.top }}>
              <div className="flex items-center gap-2.5">
                <span className="block w-1.5 h-1.5 rounded-full flex-shrink-0"
                      style={{ background: 'var(--brand-blue)', boxShadow: '0 0 0 3px rgba(46,120,200,0.30)' }} />
                <span className="h-px w-16 flex-shrink-0" style={{ background: 'linear-gradient(to right, var(--brand-blue), rgba(255,255,255,0.35))' }} />
                <span className="num-data text-[10px] uppercase whitespace-nowrap"
                      style={{ letterSpacing: '0.12em', color: 'rgba(255,255,255,0.92)', textShadow: '0 1px 7px rgba(0,0,0,0.55)' }}>{c.label}</span>
              </div>
            </div>
          ))}
            ...
            <div className="max-w-7xl mx-auto flex flex-col pt-[316px] pb-[150px] lg:pt-0 lg:pb-[104px] lg:h-full lg:justify-end">
              <div ref={contentRef} className="max-w-xl will-change-transform pointer-events-auto">
                ...
                <span
                  ref={underlineRef}
                  aria-hidden
                  className="block mt-4 h-[3px] origin-left rounded-full"
                  style={{ width: 'clamp(120px, 15vw, 230px)', background: 'var(--brand-blue)', transform: 'scaleX(0)' }}
                />

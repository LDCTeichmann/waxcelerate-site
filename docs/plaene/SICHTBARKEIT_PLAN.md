...
2. `/rewax`-Schema ergänzen, falls in `c4017cc` noch nicht geschehen: `Service` plus `Offer` mit 13,95 € und 9,95 € ab drei Ketten, plus der neuen Zehnerkarte aus `docs/aufgaben/SKILL_PREISE_UPDATE.md`, `areaServed` Deutschland, `provider` per `@id`. Prüfen, was schon da ist, nicht doppeln.
